...
watch(visible, async (value) => {
  if (!value) return
  loading.value = true
  try { ocrUrl.value = (await settingsApi.get()).settings.ocr_url || '' }
...
async function save() {
  loading.value = true
  try {
    const result = await settingsApi.update({ ocr_url: ocrUrl.value })
...
<template>
  <el-dialog v-model="visible" title="登录与 OCR 配置" width="min(560px, 92vw)">
